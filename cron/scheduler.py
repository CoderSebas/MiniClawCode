...
def schedule_job(cron: str, prompt: str, recurring: bool = True, durable: bool = True) -> CronJob | str:
    ...
    if err:
        return err
    job = CronJob(id=f'cron_{random.randint(0, 999999):06d}', cron=cron, prompt=prompt, recurring=recurring, durable=durable)
    with cron_lock:
        scheduled_jobs[job.id] = job
    if durable:
        save_durable_jobs()
...
def run_list_crons() -> str:
    ...
    if not jobs:
        return 'No cron jobs.'
    return '\n'.join(
        f"  {job.id}: '{job.cron}' -> {job.prompt[:40]} [{'recurring' if job.recurring else 'one-shot'}, {'durable' if job.durable else 'session'}]"
        for job in jobs
    )
