...
from tools.task_tools import load_task, save_task
...
def _count_worktree_changes(path: Path) -> tuple[int, int]:
    try:
        r1 = subprocess.run(['git', 'status', '--porcelain'], cwd=path, capture_output=True, text=True, timeout=10)
        files = len([l for l in r1.stdout.strip().splitlines() if l.strip()])
        r2 = subprocess.run(['git', 'log', '@{push}..HEAD', '--oneline'], cwd=path, capture_output=True, text=True, timeout=10)
